#https://wandb.ai/eliasjacob/previsao_recurso-2classes-final-full/runs/20rnld3k
# 
import argparse
import gc
import math
...
from transformers import (AdamW, AutoConfig, AutoModel, AutoModelWithLMHead,
                          AutoTokenizer, BertConfig, BertModel,
...
class TrainingModule(pl.LightningModule):
    ...
    def step(self, batch, step_name='train'):
        ...
        X_text, y = batch
        logits = self.forward(X_text).squeeze()
        y = y.type_as(logits)
    ...
    def create_data_loader(self, ds_path: str, shuffle=False):
        ...
        return DataLoader(
            JEFSuperDataset(ds_path, self.hparams.dep_var, self.hparams.text_col, self.hparams.lowercase), 
    ...
    def setup(self, stage):
        if stage == 'fit':
            ...
            self.train_steps = int(math.ceil(self.train_steps * 1))#1.04)

    def configure_optimizers(self):
        train_steps = self.train_steps
        self.lr_menor = self.lr/self.hparams.lr_divider
        named_params = list(self.model.named_parameters())

        def is_bert(name, par): return 'bert' in name and par.requires_grad
        def not_bert(name, par): return 'bert' not in name and par.requires_grad

        grouped_parameters = [
            {"params": [p for n, p in named_params if is_bert(n, p)], 'lr': self.lr},
            {"params": [p for n, p in named_params if not_bert(n, p)], 'lr': self.lr_menor},
        ]
        optimizer = torch.optim.AdamW(grouped_parameters, lr=self.lr, weight_decay=0.1)
...
def _setup_parser():
    ...
    parser.add_argument("--n_layers_lstm", type=int)
    parser.add_argument("--bidir_lstm", type=str)
    parser.add_argument("--use_special_classifier", type=str)
    parser.add_argument("--output_lstm_size", type=int)
    parser.add_argument("--lr", type=float)
    parser.add_argument("--lr_divider", type=int)
    ...
    parser.add_argument("--bert_batch_size", type=int)
    parser.add_argument("--distil_init", type=str),
...
def main():
    ...
    hparams = Namespace(
        ...
        bidir_lstm=str2bool(args.bidir_lstm),
        use_special_classifier=args.use_special_classifier,
        output_lstm_size=args.output_lstm_size,
        ...
        distil_init=str2bool(args.distil_init),
